...
const listFoods = (recipe) => {
    let result = [];
    recipe.ingredients.forEach((idx) =>{
      let temp1 = idx.slice(idx.indexOf(' ') +1)
...
const splitFoods = (recipe) => {
    let result = [];
    recipe.ingredients.forEach((idx)=> {
      result.push(idx.split(' ').slice(2).join(' '))
...
const stepActions = (recipe) => {
    let result = [];
    recipe.steps.forEach((idx) => {
        result.push(idx.split(' ').slice(0,1).join(' '))  })
    return result;
  }
